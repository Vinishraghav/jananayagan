...
- `index.html`: the landing page with an HTML5 video player.
- `styles.css`: simple styling for the page.
...
3. Open `index.html` and replace `https://example.com/your-video.mp4` with that URL.
4. Push this repo to GitHub and enable GitHub Pages on the `main` branch, serving from the root.

## Important note

GitHub cannot host a 6.33GB file directly in a normal repository, so the video must be hosted externally. This website will then play it from that external source.
